...
| 012 | Windows support: platform-neutral paths, Windows fixes, PyInstaller build in CI | Feature | L | — | IN PROGRESS 2026-09-22 on `feature/windows-support` (see `012-windows-support.md`): re-planned against the current app; CI builds an artifact, owner tests on Windows, `just release` attaches the tested zip. Steps 1–5 and 7–9 (plan, paths, Windows fixes, PyInstaller spec, Windows CI workflow) DONE; step 6 (Fusion style) waits for first Windows test; Windows CI green at `ef67131` (run 35801306497); next: owner tests that artifact (phase 4) |
