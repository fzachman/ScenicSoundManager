...
| 012 | Windows support: platform-neutral paths, Windows fixes, PyInstaller build in CI | Feature | L | — | IN PROGRESS 2026-09-22 on `feature/windows-support` (see `012-windows-support.md`): re-planned against the current app; CI builds an artifact, owner tests on Windows, `just release` attaches the tested zip. Phases 0–4 DONE (plan, paths, Windows fixes, PyInstaller spec + CI, owner test); owner test PASSED after a Windows audio fix (DirectSound output, per-player volume); step 6 not needed; next: phase 5 (`just release` attaches the tested Windows zip, install docs) |
