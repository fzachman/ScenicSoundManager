...
| 012 | Windows support: platform-neutral paths, Windows fixes, PyInstaller build in CI | Feature | L | — | DONE — shipped in v0.9.5 (2026-09-23) with `-macos` and `-windows` zips (see `012-windows-support.md`). CI builds the Windows zip, owner hand-tests it, `just release` attaches that exact artifact. Windows audio uses VLC's DirectSound output (WASAPI's volume is one shared session level). Follow-ups: 014, 015 |
...
| 014 | Windows CI hardening: fail-fast bash steps, verified VLC install with retry, job timeouts, pytest stack dumps | Infra | S | 012 | TODO, planned 2026-09-23 (see `014-windows-ci-hardening.md`); found releasing v0.9.5: choco exit 0 with no VLC + pwsh ignoring a failed preflight line → suite hung on the VLC-missing modal |
| 015 | Dependency updates without pull requests (Dependabot can't open PRs here) | Infra | S–M | — | TODO, planned 2026-09-23 (see `015-dependency-updates-without-prs.md`); needs owner pick: local `just outdated` (A), weekly reminder issue (B, recommended), or bot-only PRs (C); 7 pins + 2 action majors behind as of 2026-09-23 |
