...
- **Theming**: Palette tokens live in `Styles` (`app/shared/styles.py`) and are swapped by `Styles.set_theme()`; never hardcode colors — use tokens, `Styles.tint()` for alpha overlays, and `Styles.contrast_text_color()` for text on arbitrary/accent backgrounds. Icons come from `IconLibrary.icon(name, color)` (SVGs use `currentColor`; tint is baked at render). Persistent widgets consolidate palette-dependent `setStyleSheet`/`setIcon` calls into `_apply_theme_styles()`, called at init and connected once to `theme_manager.theme_changed` (`app/shared/theme.py`); transient dialogs need no wiring. Theme swaps must be style-only — never mutate playback, selection, or checked state, and never reload from the DB.
- **Logging**: Use `structlog` via `app.shared.logging.get_logger(__name__)`. Info+ goes to `soundmanager.log` in `app.paths.LOG_DIR` (`~/Library/Logs/ScenicSound/` on macOS; rotated); the console only shows warning+.
- **Paths**: Per-platform data/log folders live in `app/paths.py` (stdlib-only — logging needs them before `QApplication` exists). macOS paths are pinned by tests because beta users' data lives there; never move them without a migration.
